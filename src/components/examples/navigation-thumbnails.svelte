...
<div class="bg-embla-carousel p-4 rounded-[.25rem] overflow-hidden">
		...
		<div class="grid grid-flow-col auto-cols-[100%] grid-rows-[190px] gap-x-[10px]">
			{#each images as src, index}
				<div class="relative rounded-[.25rem] overflow-hidden">
					<p class="absolute top-2 right-[9px] grid place-items-center bg-embla-carousel rounded-full text-svelte w-12 h-12">{index + 1}</p>
	...
	<div class="overflow-hidden mt-[10px]" use:embla={{ dragFree: true, align: "start", containScroll: "keepSnaps", store: thumbnails }}>
		<div class="grid grid-flow-col auto-cols-[89px] grid-rows-[70px] gap-x-[10px]">
			{#each images as src, index}
				<button class="group relative rounded-[.25rem] overflow-hidden" disabled={selected === index} on:click={onThumbnailSelect(index)}>
					<img class="w-full h-full object-cover" {src} alt="">

					{#if selected === index}
						<div class="absolute top-0 left-0 w-full h-full bg-black opacity-60" transition:fade={{ duration: 250 }} />
					{/if}

					<p class="absolute top-1 right-1 grid place-items-center bg-embla-carousel rounded-full text-svelte w-8 h-8">{index + 1}</p>
				</button>
			{/each}
		</div>
	</div>
